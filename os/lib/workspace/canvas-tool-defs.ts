...
export const canvasToolDefs: CanvasToolDef[] = [
          ...
          items: {
            ...
            properties: {
              ...
              rationale: {
                type: "string",
                description:
                  "Explicit per-bridge reasoning. MUST cover: (1) the user gesture that triggers it, (2) why this exact source port fires on that gesture, (3) why the target needs this live input instead of relying on its own binding, (4) schema compatibility (identity or an existing listed transform). One or two sentences is fine, but all four points must be present.",
              },
            },
  ...
  },
  {
    ...
    description:
      "Return the current live canvas state: nodes, edges, outputs, layout, widget contracts, and existing transforms.",
  ...
  },
  {
    ...
    description:
      "Create a bridge from a source output port to a target input port. Returns the bridge id or a rejection reason. Before calling, you must have walked through the six bridge-reasoning checks: (1) a real user gesture triggers it, (2) the source port fires on that gesture, (3) the target port exists in its contract, (4) schemas match or an existing listed transform bridges them, (5) the target genuinely needs live input (not just its binding), (6) no feedback loop. If schemas differ and no listed transform fits, skip this bridge. Wrong bridges are worse than missing ones.",
    parameters: {
      type: "object",
      properties: {
        id: { type: "string" },
  ...
  },
  {
    ...
    description: "List existing transforms with their input/output schemas.",
    parameters: { type: "object", properties: {}, additionalProperties: false },
  },
];
