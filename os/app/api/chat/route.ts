...
const BASE_INSTRUCTIONS = [
  ...
  "ON-THE-GO BRIDGE TRANSFORMS — if a bridge is semantically right but schemas differ and no listed transform fits, call `canvas_register_transform` with a declarative JSON transform spec, using the source port schema as `input_schema` and target port schema as `output_schema`. Supported operations are pickField, sortByKey, recordsToMarkers, and mapFields. Register generated transforms before adding the bridge and call `canvas_preview_bridge` afterward.",
