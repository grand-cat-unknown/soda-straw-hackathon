...
const BASE_INSTRUCTIONS = [
  ...
  "BRIDGE REASONING — bridges are the fragile part of the workspace, so think hard before you wire one. Before you announce or add ANY bridge, deliberately reason through these questions and state your reasoning briefly in the plan announcement: (a) What concrete user interaction triggers this bridge? Name the gesture — e.g. \"user clicks a row in the contacts table\". If you can't name the gesture, do not add the bridge. (b) Which exact output port on the source widget fires from that gesture? Look it up in the source widget's contract; do not guess port names. (c) Which exact input port on the target widget consumes it? Look it up in the target contract. (d) Are the two schemas the SAME shape, or do they need a transform? If shapes differ, either pick a registered transform (call `canvas_list_transforms` if unsure) or drop the bridge. Never wire a bridge hoping a transform exists. (e) Does the target widget actually need this live input, or is its `bindings` entry already sufficient? Do not bridge data the target already loads itself. (f) Will this bridge cause a feedback loop or overwrite a user edit (e.g. bridging back into a port that the user just typed into)? If yes, drop it. Only bridges that pass all six checks belong in the plan.",
  "PREFERRED BRIDGE PATTERNS — these are known-stable shapes you should reach for first: source-list `selectedRow`/`selectedRows`/`selectedMarker`/`selectedItem` -> detail widget's singular `row`/`item`/`marker`/`contact` input with `transform: 'identity'`. Filter/search widget's `query` output -> list widget's `filter` input. Avoid wiring list-output ports (plural `contacts`, `rows`, `items`) into other list inputs — those collections are owned by bindings, not bridges. Avoid bridging into ports whose schema you have not seen in the contract block below.",
