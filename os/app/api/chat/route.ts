...
const BASE_INSTRUCTIONS = [
  ...
  "You also have live canvas mutation tools that operate directly on the workspace the user is looking at: `canvas_announce_plan`, `canvas_get_state`, `canvas_list_widgets`, `canvas_add_widget`, `canvas_update_widget_input`, `canvas_remove_widget`, `canvas_set_layout`, `canvas_add_bridge`, `canvas_remove_bridge`, `canvas_preview_bridge`, `canvas_list_bridge_suggestions`.",
  ...
  "Use the provided capability catalog, widget contracts, and current canvas snapshot to decide what to ask. Do not ask generic questions whose answers would not change the workspace.",
  ...
  "BRIDGE REASONING — only wire bridges for real live UI interactions. Before announcing or adding a bridge, confirm the user gesture, exact source output port, exact target input port, compatible schemas, real need for live input, and no feedback loop. If any part is uncertain, skip the bridge.",
  "PREFERRED BRIDGE PATTERNS — use singular selection outputs for detail widgets, multi-selection outputs for visualizations that accept arrays, and search/filter outputs for list filters. Avoid bridging loaded collections that already come from widget bindings.",
  "After every incremental `canvas_add_widget` (not part of a freshly-announced plan), call `canvas_list_bridge_suggestions` with the new node id, then judge each suggestion before calling `canvas_add_bridge`. Skip suggestions that are only structurally valid but not useful.",
  ...
  "Before adding a bridge, you may call `canvas_preview_bridge` to verify port compatibility and inspect what the target would receive. If preview fails, skip the bridge.",
